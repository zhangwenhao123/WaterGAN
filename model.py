...
class WGAN(object):
  def __init__(self, sess, input_height=640, input_width=480, input_water_height=1360, input_water_width=1024, is_crop=True,
         ...
         depth_dataset_name='default',input_fname_pattern='*.*', checkpoint_dir=None, results_dir=None, sample_dir=None,num_samples=4000):
    """
  ...
  def train(self, config):
    ...
    for epoch in xrange(config.epoch):
      ...
      for idx in xrange(0, (water_batch_idxs*config.batch_size), config.batch_size):
        ...
        errG_ori = self.g_loss_ori.eval({self.z:batch_z,self.air_inputs: air_batch_images,self.depth_inputs:depth_batch_images,self.R2:r2, self.R4:r4, self.R6:r6})

        counter += 1
        print("Epoch: [%2d] [%4d/%4d] time: %4.4f, d_loss: %.8f, g_loss: %.8f,g_loss_ori: %.8f"\
...
##############################################################
  ...
  def wc_generator(self, z, image, depth,r2,r4,r6, y=None):
    with tf.variable_scope("wc_generator") as scope:
      ...
      with tf.variable_scope("g_atten"):
          ### attenuation coefficient  is randomly set 
          ...
          eta = tf.stack([eta_r,eta_g,eta_b],axis=3)   ## change
  ...
  def read_depth(self, filename):
    ...
    depthtmp=depth_mat["Img"]       # smoothedDepth or depth
    ds = depthtmp.shape
    if self.is_crop:
